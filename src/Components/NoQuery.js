import React from 'react';

const NoQuery = props => {

  let queries = [
    'news',
    'sports',
    'finance',
    'entertainment',
    'economy',
    'f1',
    'space',
    'politics',
    'health'
  ]

  let query = queries[Math.floor(Math.random() * queries.length)];
  let url = "/?q=" + query + "&page=1"

  return (
    <div>
      <h5><strong>DatoRSS – RSS Search Engine</strong></h5>
      <p>
      <h5>Fast, Awesome, Clean and Powerful RSS search engine, browse through over <strong>800,000</strong> RSS posts. </h5>
      </p>
      <a className="btn btn-dark" href="https://github.com/davidesantangelo/feedi" target="_BLANK">RESTful API <i className="fas fa-code"></i></a>  &nbsp;
      <a href="https://docs.google.com/spreadsheets/d/1mVeA-tzWNTypBTWy0bgijKYrc3OkJ8VX1uKOhC9aKnk/edit#gid=0" target="_BLANK" className="btn btn-secondary">Sources <i className="fas fa-share-alt"></i></a>
    </div>
  );
  
}

export default NoQuery;